...
class Config {
	...
	public static function createKhaFile(config:ConfigData):String {
		...
		kfile += 'p.addShaders("${Path.join([CLI.engineDir, 'resources/shaders'])}");\n';

		// inputs
		if(config.input != null) {
			if(config.input.mouse) {
				kfile += 'p.addDefine("nuc_mouse_input");\n';
			}
			if(config.input.keyboard) {
				kfile += 'p.addDefine("nuc_keyboard_input");\n';
			}
			if(config.input.gamepad) {
				kfile += 'p.addDefine("nuc_gamepad_input");\n';
			}
			if(config.input.touch) {
				kfile += 'p.addDefine("nuc_touch_input");\n';
			}
			if(config.input.pen) {
				kfile += 'p.addDefine("nuc_pen_input");\n';
			}
		} else {
			kfile += 'p.addDefine("nuc_mouse_input");\n';
			kfile += 'p.addDefine("nuc_keyboard_input");\n';
		...
		if(resourcesPath != null && resourcesPath.length > 0) {
			kfile += 'p.addDefine("nuc_resources_path=$resourcesPath");\n';
	...
	public static function get():ConfigData {
		var configPath = Path.join([CLI.userDir, 'project.nuc']);
		if (!FileSystem.exists(configPath)) {
			CLI.error('Cant find project.nuc in: ${CLI.userDir}');
		}
		var data = File.getContent(configPath);
		var config:ConfigData = NucML.parse(data);
