...
class Init extends Command {
	...
	function createProject(template:String) {
		var templatePath = Path.join([CLI.engineDir, CLI.resourcesPath, CLI.templatesPath, template]);
		if (!FileSystem.exists(templatePath)) CLI.error('Cant find ${template} template');
		
		var dir = FileSystem.readDirectory(CLI.userDir);
		if(dir.length > 0) CLI.error('${CLI.userDir} folder is not empty');
