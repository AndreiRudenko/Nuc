...
class Build extends Command {
	...
	public function new() {
		super(
			'build', 
			'build current project: build <target> [--compile, --debug, --onlydata, --noshaders, --watch]'
		);
	}

	override function execute(args:Array<String>) {
		if(args.length == 0) CLI.error('Target not defined');
		...
		for (a in args) {
			switch (a) {
				...
				case '--onlydata': config.onlydata = true;
        ...
        File.saveContent(Path.join([buildPath, 'khafile.js']), khafile);
	...
	function buildProject(config:ConfigData) {
		...
		if(compiler != null) {
			...
			if(compiler.main != null && compiler.main != '') {
				args.push('--main');
				args.push(compiler.main);
			}
		}

		args.push('--projectfile');
		args.push('build/khafile.js');
		...
		if(config.watch) {
			args.push('--watch');
			res = CLI.execute('start', ['cmd', "/c", '${CLI.khamakePath}'].concat(args));
		} else {
			res = CLI.execute(CLI.khamakePath, args);
		}
		// var res = CLI.execute(CLI.khamakePath, args);
		if(res != 0) CLI.error('Build failed'); 
		
		if(config.target == 'html5') postbuildHtml5(config);
	...
	function postbuildHtml5(config:ConfigData) {
		if(config.html5 != null) {
			...
			if(config.html5.htmlFile != null && config.html5.htmlFile != '') {
				...
				htmlPath = Path.join([CLI.engineDir, CLI.resourcesPath, 'html/index.html']);
			}
