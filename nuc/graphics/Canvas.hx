...
class Canvas {
	...
	public var transform(get, set):FastAffine;
	var _transform:FastAffine;
	inline function get_transform() return _transform; 
	function set_transform(v:FastAffine):FastAffine {
		return _transform.copyFrom(v);
	}

	var _transformStack:Array<FastAffine>;
	var _transformPool:DynamicPool<FastAffine>;

	var _invertseTransform:FastAffine;
	var _invertseTransformDirty:Bool = true;

	public function new() {
		_transform = new FastAffine();
		_invertseTransform = new FastAffine();
		_transformStack = [];
		_transformPool = new DynamicPool<FastAffine>(16, function() { return new FastAffine(); });
	}
	
	public function pushTransform(?m:FastAffine) {
	...
	public function applyTransform(m:FastAffine) {
		_transform.append(m);
		onTransformUpdate();
	}
