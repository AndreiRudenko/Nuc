...
class Mouse {
	...
	// public static var all(default, null):Array<Mouse> = [];
	
	// static public function get(index:Int):Mouse {
	// 	return all[index];
	// }
	...
	public function enable() {
		...
		if(m != null) m.notify(_onPressed, _onReleased, _onMove, _onWheel);
		#end
	...
	public function disable() {
		...
		#if nuc_mouse_input
		var m = kha.input.Mouse.get();
		if(m != null) m.remove(_onPressed, _onReleased, _onMove, _onWheel);
		#end
	...
	function checkBinding(key:Int, pressed:Bool) {
		for (k in _mouseBindings.keys()) {
			if(_mouseBindings.exists(k)) {
				var n = _mouseBindings.get(k);
				if(Bits.check(n, key)) {
					_binding.inputEvent.setMouseEvent(k, _mouseEvent);
					if(pressed) {
	...
	function _onPressed(button:Int, x:Int, y:Int) {
		...
		Nuc.app.emitter.emit(MouseEvent.MOUSE_DOWN, _mouseEvent);
	}
	...
	function _onReleased(button:Int, x:Int, y:Int) {
		...
		Nuc.app.emitter.emit(MouseEvent.MOUSE_UP, _mouseEvent);
	}
	...
	function _onWheel(d:Int) {
		...
		Nuc.app.emitter.emit(MouseEvent.MOUSE_WHEEL, _mouseEvent);
	}
	...
	function _onMove(x:Int, y:Int, dx:Int, dy:Int) {
		...
		Nuc.app.emitter.emit(MouseEvent.MOUSE_MOVE, _mouseEvent);
	}
