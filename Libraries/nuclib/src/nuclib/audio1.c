...
static float sample(struct NucAudioChannel *channel, int position, int sampleRate) {
	...
	if (even) {
        ...
        float d1 = channel->data[pos1];
        float d2 = channel->data[pos2];
        
		float a = (pos1 < minimum || pos1 > maximum) ? 0 : d1;
		float b = (pos2 < minimum || pos2 > maximum) ? 0 : d2;
...
void NucAudioChannel_nextSamples(struct NucAudioChannel *channel, float *requestedSamples, int requestedLength, int sampleRate) {
	...
	while (requestedSamplesIndex < requestedLength) {
		...
		for (int i = 0; i < iterations; ++i) {
            float v = sample(channel, position++, sampleRate);
			requestedSamples[requestedSamplesIndex++] = v;
		}
