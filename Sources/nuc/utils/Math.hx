...
class Math {
	...
	static public inline function imod(i:Int, n:Int):Int {
		return (i % n + n) % n;
	}

	static public inline function mod(n:Float, d:Float):Float {
	...
	static public inline function nextPowOf2(x:Int):Int {
		x |= x >> 1;
		x |= x >> 2;
		x |= x >> 4;
		x |= x >> 8;
	...
	static public inline function prevPowOf2(x:Int):Int {
		x |= x >>> 1;
		x |= x >>> 2;
		x |= x >>> 4;
		x |= x >>> 8;
	...
	static public inline function checkPowOf2(x:Int):Bool {
		return x != 0 && (x & (x - 1)) == 0;
	}

	static public inline function getPowOf2(x:Int):Int {
		if(x == 0) {
			return 1;
		}
